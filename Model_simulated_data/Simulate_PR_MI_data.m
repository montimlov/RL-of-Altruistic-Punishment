...
%% Jialu's note
%parameter recovery:参数反真性，衡量模拟值和真值有多接近
...
models{1} = 'model_RWPL_temp_bias'; % enter model to run here **
models{2} = 'model_RWPL_SO_LR'; % enter model to run here **
models{3} = 'model_RWPL_SON_LR_SON_beta'; % enter model to run here **
...
for tr = toRun
    ...
    for r = 1:nRounds
        ...
        for m = modelsTR % loop over model number(s) specified above
            ...
            if type == 1 % if using a grid of values
                grid.alpha  = [0.1:0.2:0.9]; % define grid values **
                grid.beta   = [0.1:0.2:0.9]; % define grid values **
                grid.bias = 0.15;
                noise = 0.1; % level of noise added to grid of starting values **
            ...
            if r == 1 % plot for the first round only to avoid lots of figures
                ...
                for param=1:nParam
                    ...
                    elseif contains(thisp, 'bias') == 1
                        histogram(norm2bias(allCombsNorm(:,param)+6*noise*randn(length(allCombsNorm),1)),'FaceColor',[0.5 0.5 0.5]);
                    end
            ...
            % -------------------------------------------- %
            % Jialu's note:这个循环就是在随机生成组块的参数组合，并用这些随机生成的参数组合来拟合个体表现数据
            for simS=1:nSubj
                allbl=[];
                if simS < 0.5*nSubj
                ...
                % Jialu's note: 这里是用模拟函数，根据给定参量生成虚假被试的行为表现
                if simS==1458
                    ssss=1;
                end
                try
                ...
                s.PL.beh{1,simS}.startvalueA = allout.all_Va(1);
                s.PL.beh{1,simS}.startvalueB = allout.all_Vb(1);
                s.PL.ID{1,simS}.ID = simS;
                s.PL.beh{1,simS}.bias = norm2bias(truep(7));
            ...
            if tr == 1 && mle_em == 2 % if using em for PR, fit all subjects
                ...
                param_new = params(1:6);
                [s, p] = em_PR(s, models(modelsTR), param_new);
                for simS=1:nSubj
            ...
            if tr == 1 % for PR create plots then confusion matrix & save
                ...
                for simS=1:nSubj
                    trueParam = [trueParam;Data(simS).trueParam(1:6)];
                    fittedParam = [fittedParam;Data(simS).fittedParam];
                    if isempty(Data(simS).fittedParam)
                        %disp('empty');
                        missParam = [missParam;Data(simS).trueParam(1:6)'];
                        trueParam(end,1:6)=[];
                    end
                end
                ...
                for param=1:nParam % plot correlations
                    ...
                    elseif contains(thisp, 'beta') == 1
                        ...
                        all_corr(param,:) = corr(trueAlphaBetas(:,param),fittedAlphaBetas(:,param));
                    elseif contains(thisp, 'bias') == 1
                        trueAlphaBetas(1:fitSubj,param) = norm2bias(trueParam(:,param));
                        fittedAlphaBetas(1:fitSubj,param) = norm2bias(fittedParam(:,param));
                        plot(trueAlphaBetas(:,param),fittedAlphaBetas(:,param),'k.','MarkerSize',12);
                        all_corr(param,:) = corr(trueAlphaBetas(:,param),fittedAlphaBetas(:,param));
                        xlim([0,1]);ylim([0,1]);
                    end
    ...
    if type == 1
        name = ['../Prosocial_learning_R_code/',toRunOptions{tr},'_',mleOptions{mle_em},'_a_',num2str(grid.alpha(1)),'_',num2str(grid.alpha(end)),'_b_',num2str(grid.beta(1)),'_',num2str(grid.beta(end)),'_n_',num2str(noise),'.mat'];
    elseif type == 2
        name = ['../Prosocial_learning_R_code/',toRunOptions{tr},'_',mleOptions{mle_em},'_',num2str(nSubj),'_subjects_',num2str(nRounds),'_rounds_from_distrib'];
    end
    save(name)
