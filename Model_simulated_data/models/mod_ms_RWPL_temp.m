function [fval,fit] = mod_ms_RWPL_temp(behavData,q, doprior,dofit,varargin)
%%
% -------------------------------------------------------------------------------------
...
qt = norm2par('ms_RWPL_temp',q); % transform parameters from gaussian space to model space
...
for iagent=1:num_cond; %%%% loop through the 3 conditions, self, other, no one
    ...
    for t=1:num_pic_reps  % for the 16 reps of the stimuli                                               % loop through every presentation of the same stimulus
        ...
        if t <= 15
            ...
            elseif agentt(t) ==2 %|| Jialu's note: for outgroup
...
if dofit ==1
   ...
   fit         = struct;
   fit.xnames  = {'beta_self'; 'beta_other';'alpha_self_front'; 'alpha_other_front';'alpha_self_later';'alpha_other_later'};
