...
function [f, allout] = model_RWPL_SON_LR_SON_beta_simulate(data,normalPE,block,agent,p,alphabounds,betabounds)
...
beta_self   = norm2beta(p(3));
beta_other   = norm2beta(p(4));

all_alphas = [alpha_self, alpha_other];
...
freqB = [0.193,0.193,0.288,0.291,0.392,0.402,0.533,0.533,0.673,0.676];
...
%             if ~isnan(choicet(t)), probs_choice(t)=softmax_num/softmax_denom; end;
            ...
            probs_choice(t) = exp(Va(t)/allbetas(iagent))/(exp(Va(t)/allbetas(iagent))+exp(Vb(t)/allbetas(iagent)));
            ...
            if choicet(t)== 1 % if choose the high reward option
                ...
                Va(t+1) = Va(t) +  (all_alphas(iagent)*PE(t));
            end
            ...
            if choicet(t)== 0 % if choose the low reward option
                ...
                Vb(t+1) = Vb(t) +  (all_alphas(iagent)*PE(t));
            end
